...
- (void)setNotebook:(Notebook *)notebook{
    _notebook = notebook;
    self.selectionPages = nil;
    self.selectionPages = [[NSMutableArray alloc] init];
    for (Page *page in notebook.pages) {
        PageSelectionInfo *pageSelectionInfo = [PageSelectionInfo new];
        pageSelectionInfo.page = page;
        pageSelectionInfo.selected = NO;
        [self.selectionPages addObject:pageSelectionInfo];
    }
}
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [UIColor colorWithRed:40/255.0 green:44/255.0 blue:55/255.0 alpha:1.0];
    ...
    _toolbar.barTintColor = [UIColor colorWithRed:34/255.0 green:37/255.0 blue:46/255.0 alpha:1.0];
    ...
    if (self.displayNavArrows) {
        ...
        UIImage *previousButtonImage = [UIImage imageNamed:[NSString stringWithFormat:arrowPathFormat, @"Left"]];
        UIImage *nextButtonImage = [UIImage imageNamed:[NSString stringWithFormat:arrowPathFormat, @"Right"]];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        if (!self.addButton) {
            ...
            [self.addButton setFrame:CGRectMake(screenRect.size.width - 80, screenRect.size.height - 100, 64, 64)];
            self.addButton.backgroundColor = [UIColor colorWithRed:34/255.0 green:37/255.0 blue:46/255.0 alpha:1.0];
            self.addButton.layer.cornerRadius = self.addButton.frame.size.width / 2.0;
            [self.addButton setClipsToBounds:YES];
            [self.addButton setImage:[UIImage imageNamed:@"btn_add"] forState:UIControlStateNormal];
            [self.addButton setImage:[UIImage imageNamed:@"btn_add_hover"] forState:UIControlStateHighlighted];
            [self.addButton setBackgroundImage:[UIImage imageNamed:@"btn_add_notes_bg"] forState:UIControlStateNormal];
            [self.addButton setBackgroundImage:[UIImage imageNamed:@"btn_add_notes_bg_hover"] forState:UIControlStateHighlighted];
            [self.addButton setImageEdgeInsets:UIEdgeInsetsMake(22, 22, 22, 22)];
            [self.addButton addTarget:self action:@selector(scan) forControlEvents:UIControlEventTouchUpInside];
            [self.view addSubview:self.addButton];
            
            self.deleteButton = [UIButton buttonWithType:UIButtonTypeCustom];
            [self.deleteButton setFrame:self.addButton.frame];
...
- (void)delete{
    ...
    for (int i = 0; i < self.selectionPages.count; i++) {
        ...
        if (pageSelectionInfo.selected) {
            NSLog(@"selected");
            [tempSet removeObject:pageSelectionInfo.page];
        }else{
            NSLog(@"not selected");
        }
    }
    ...
    [self.delegate refreshPhotoBrowserWithNotebook:self.notebook];
    [self edit];
...
- (void)scan {
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main"
                                                  bundle:nil];
    CameraViewController *cameraViewController = (CameraViewController *)[storyboard instantiateViewControllerWithIdentifier:@"CameraViewController"];
...
- (void)notebookDidChange:(Notebook *)notebook {
    [self.delegate refreshPhotoBrowserWithNotebook:notebook];
    [_toolbar setAlpha:0.0];
}
